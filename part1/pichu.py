...
def updatePos(board, xPos1, yPos1, xPos2, yPos2, charToReplace, message):
	...
	state = {'board': flatList, 'action': message}
	return state


def isEnemy(board, x,  y):
...
def successors(board):
	...
	if player == 'w':
		...
		for possiblePositions in range(0, len(pawnPos), 1):
			...
			if (rowCounter + 1) <= 7:
				if (currentState[rowCounter + 1][colCounter].islower()) or currentState[rowCounter + 1][colCounter] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter + 1, colCounter, 'P', 'Pawn at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+1+1)+' column '+str(colCounter+1)))
			if rowCounter == 1:
				if currentState[rowCounter + 1][colCounter] == '.' and currentState[rowCounter + 2][colCounter] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter + 2, colCounter, 'P', 'Pawn at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+2+1)+' column '+str(colCounter+1)))
			# move pawn left
			if (colCounter - 1) >= 0:
				if (currentState[rowCounter][colCounter - 1].islower()) or currentState[rowCounter][colCounter - 1] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter, colCounter - 1, 'P', 'Pawn at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+1)+' column '+str(colCounter-1+1)))
			# move pawn right
			if (colCounter + 1) <= 7:
				if (currentState[rowCounter][colCounter + 1].islower()) or currentState[rowCounter][colCounter + 1] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter, colCounter + 1, 'P', 'Pawn at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+1)+' column '+str(colCounter+1+1)))
			...
			if (colCounter - 1) >= 0 and (rowCounter + 1) <= 7:
				if (currentState[rowCounter + 1][colCounter - 1].islower()):
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter + 1, colCounter - 1, 'P', 'Pawn at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+1+1)+' column '+str(colCounter-1+1)))
		...
		for possiblePositions in range(0, len(rookPos), 1):
			...
			for i in range(rowCounter - 1, -1, -1):
				if (currentState[i][colCounter].isupper()):
					...
					successor.append(updatePos(currentState, rowCounter, colCounter, i, colCounter, 'R','Rook at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(i+1)+' column '+str(colCounter+1)))
					break
			...
			for i in range(colCounter - 1, -1, -1):
				if (currentState[rowCounter][i].isupper()):
					...
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter, i, 'R','Rook at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+1)+' column '+str(i+1)))
					break
			...
			for r in range(rowCounter, -1, -1):
				if ((rowCounter - r) >= 0 and (colCounter - r) >= 0):
					if (currentState[rowCounter - r][colCounter - r].isupper()):
						...
						successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter - r, colCounter - r, 'B','Bishop at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter-r+1)+' column '+str(colCounter-r+1)))
						break
			...
			for r in range(rowCounter, 8, 1):
				if ((rowCounter + i) < 8 and (colCounter + i) < 8):
					if (currentState[rowCounter + i][colCounter + i].isupper()):
						...
						successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter + i, colCounter + i, 'B','Bishop at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+i+1)+' column '+str(colCounter+i+1)))
						break
		...
		for possiblePositions in range(0, len(queenPos), 1):
			...
			for r in range(rowCounter, -1, -1):
				if ((rowCounter - r) >= 0 and (colCounter - r) >= 0):
					if (currentState[rowCounter - r][colCounter - r].isupper()):
						...
						successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter - r, colCounter - r, 'Q','Queen at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter-r+1)+' column '+str(colCounter-r+1)))
						break
					elif currentState[rowCounter - r][colCounter - r] == '.':
						successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter - r, colCounter - r, 'Q','Queen at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter-r+1)+' column '+str(colCounter-r+1)))
					else:
			...
			for r in range(rowCounter, 8, 1):
				if ((rowCounter + i) < 8 and (colCounter + i) < 8):
					if (currentState[rowCounter + i][colCounter + i].isupper()):
						...
						successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter + i, colCounter + i, 'Q','Queen at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+i+1)+' column '+str(colCounter+i+1)))
						break
					elif currentState[rowCounter + i][colCounter + i] == '.':
						successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter + i, colCounter + i, 'Q','Queen at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+i+1)+' column '+str(colCounter+i+1)))
					else:
			...
			for i in range(rowCounter - 1, -1, -1):
				if (currentState[i][colCounter].isupper()):
					...
					successor.append(updatePos(currentState, rowCounter, colCounter, i, colCounter, 'Q','Queen at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(i+1)+' column '+str(colCounter+1)))
					break
				elif currentState[i][colCounter] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, i, colCounter, 'Q','Queen at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(i+1)+' column '+str(colCounter+1)))
				else:
			...
			for i in range(colCounter - 1, -1, -1):
				if (currentState[rowCounter][i].isupper()):
					...
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter, i, 'Q','Queen at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+1)+' column '+str(i+1)))
					break
				elif currentState[rowCounter][i] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter, i, 'Q','Queen at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+1)+' column '+str(i+1)))
				else:
		...
		for possiblePositions in range(0, len(kingPos), 1):
			...
			if (rowCounter - 1) >= 0:
				if (currentState[rowCounter - 1][colCounter].islower()) or currentState[rowCounter - 1][colCounter] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter - 1, colCounter, 'K','King at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter-1+1)+' column '+str(colCounter+1)))
			# move king down
			if (rowCounter + 1) <= 7:
				if (currentState[rowCounter + 1][colCounter].islower()) or currentState[rowCounter + 1][colCounter] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter + 1, colCounter, 'K','King at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+1+1)+' column '+str(colCounter+1)))
			...
			if (colCounter - 1) >= 0:
				if (currentState[rowCounter][colCounter - 1].islower()) or currentState[rowCounter][colCounter - 1] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter, colCounter - 1, 'K','King at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+1)+' column '+str(colCounter-1+1)))
			# move king right
			if (colCounter + 1) <= 7:
				if (currentState[rowCounter][colCounter + 1].islower()) or currentState[rowCounter][colCounter + 1] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter, colCounter + 1, 'K','King at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+1)+' column '+str(colCounter+1+1)))
			...
			if (rowCounter + 1) <= 7 and (colCounter - 1) >= 0:
				if (currentState[rowCounter + 1][colCounter - 1].islower()) or currentState[rowCounter + 1][colCounter - 1] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter + 1, colCounter - 1, 'K','King at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter-1+1)+' column '+str(colCounter-1+1)))
		...
		for possiblePositions in range(0, len(knightPos), 1):
			...
			if (rowCounter - 1) >= 0 and (colCounter + 2) <= 7:
				if (currentState[rowCounter - 1][colCounter + 2].islower()) or currentState[rowCounter - 1][colCounter + 2] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter - 1, colCounter + 2, 'N','Knight at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter-1+1)+' column '+str(colCounter+1+2)))
			if (rowCounter - 2) >= 0 and (colCounter + 1) <= 7:
				if (currentState[rowCounter - 2][colCounter + 1].islower()) or currentState[rowCounter - 2][colCounter + 1] == '.':
					successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter - 2, colCounter + 1, 'N','Knight at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter-2+1)+' column '+str(colCounter+1+1)))
			...
			for r in range(rowCounter, 8, 1):
				if ((rowCounter + i) < 8 and (colCounter + i) < 8):
					if (currentState[rowCounter + i][colCounter + i].islower()):
						...
						successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter + i, colCounter + i, 'q', 'Q','Queen at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+i+1)+' column '+str(colCounter+i+1)))
						break
					elif currentState[rowCounter + i][colCounter + i] == '.':
						successor.append(updatePos(currentState, rowCounter, colCounter, rowCounter + i, colCounter + i, 'q', 'Q','Queen at row '+str(rowCounter+1)+' column '+str(colCounter+1)+' to row '+str(rowCounter+i+1)+' column '+str(colCounter+i+1)))
					else:
...
def decision():
	...
	if not is_terminal(initialBoard):
		...
		if evaluation(initialBoard) > 0:
			...
			initialBoard['action'] = "You have already Lost"
			return initialBoard
...
initialBoard = {'board': inputBoard, "action": ""}
